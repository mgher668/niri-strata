...
Item {
    ...
    // ── Portal/gsettings export ──

    function exportPortal() {
        var isLight = Theme._isLight;
    ...
    // ── niri colors.kdl export ──

    function exportNiri() {
        var cd = configDir();
        var niriDir = cd + "/niri/strata";
        var kdlContent = _renderNiriKdl();

        var cmd = "set -euo pipefail\n"
            + "mkdir -p '" + niriDir + "'\n"
            + "cat > '" + niriDir + "/colors.kdl' << 'NIRI_EOF'\n"
            + kdlContent
            + "NIRI_EOF\n";

        niriProcess.command = ["sh", "-c", cmd];
        niriProcess.running = true;
    }

    function _renderNiriKdl() {
        var c = Theme.colors;
        var isLight = Theme._isLight;
        var lines = [
