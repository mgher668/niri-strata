...
test("wires launcher services and overlay through shell", async () => {
  ...
  assert.match(appSearch, /import Quickshell\.Io/);
  assert.match(appSearch, /readonly property string helperScriptUrl:\s*String\(Qt\.resolvedUrl\("\.\.\/\.\.\/scripts\/launcher-indexer\.mjs"\)\)/);
  assert.match(appSearch, /readonly property var applications:\s*fallbackCacheLoaded \? DesktopEntries\.applications\.values : \[\]/);
  assert.match(appSearch, /property var queryResults:\s*\(\{\}\)/);
  assert.match(appSearch, /property var helperApps:\s*\[\]/);
  assert.match(appSearch, /property var lastStableResults:\s*\[\]/);
  ...
  assert.match(appSearch, /property bool fallbackCacheLoaded:\s*false/);
  assert.match(appSearch, /property bool refreshingCache:\s*false/);
  assert.match(appSearch, /property bool helperAvailable:\s*true/);
  assert.match(appSearch, /property bool helperSearchRunning:\s*false/);
  assert.doesNotMatch(appSearch, /Component\.onCompleted:\s*refreshCache\(\)/);
  assert.match(appSearch, /onApplicationsChanged:\s*\{[\s\S]*if \(fallbackCacheLoaded && !refreshingCache\)[\s\S]*refreshCache\(\);[\s\S]*\}/);
  assert.match(appSearch, /function startWatcher\(\)[\s\S]*watcherProcess\.exec\(helperCommand\(\["watch", "--quiet"\]\)\)/);
  assert.match(appSearch, /function ensureCache\(\)[\s\S]*startWatcher\(\);[\s\S]*requestSearch\(""\)/);
  assert.match(appSearch, /function requestSearch\(query\)/);
  assert.match(appSearch, /function runPendingSearch\(\)[\s\S]*searchProcess\.exec\(helperCommand\(\["search", "--query", activeQuery\]\)\)/);
  assert.match(appSearch, /function applySearchPayload\(payload\)[\s\S]*next\[key\] = payload\.results[\s\S]*if \(key === ""\)[\s\S]*helperApps = payload\.results[\s\S]*lastStableResults = payload\.results[\s\S]*revision \+= 1/);
  assert.match(appSearch, /function search\(query, limit\)[\s\S]*const key = String\(query \?\? ""\)[\s\S]*queryResults\[key\][\s\S]*helperApps\.length > 0[\s\S]*cachedSearch\(key, helperApps, limit\)[\s\S]*results = lastStableResults/);
  assert.match(appSearch, /function launch\(result\)[\s\S]*launchProcess\.exec\(helperCommand\(\["launch", "--app-id", String\(result\.appId\)\]\)\)/);
  assert.match(appSearch, /Process\s*\{[\s\S]*id:\s*searchProcess[\s\S]*StdioCollector/);
  assert.match(appSearch, /Process\s*\{[\s\S]*id:\s*launchProcess/);
  assert.match(appSearch, /Process\s*\{[\s\S]*id:\s*watcherProcess/);
  assert.match(appSearch, /function refreshCache\(\)/);
  assert.match(appSearch, /refreshingCache = true/);
  assert.match(appSearch, /fallbackCacheLoaded = true/);
  ...
  assert.match(appSearch, /function cachedSearch\(query, apps, limit\)/);
  assert.match(appSearch, /function fallbackSearch\(query, limit\)[\s\S]*return cachedSearch\(query, normalizedApplications\(\), limit\)/);
  assert.doesNotMatch(appSearch.match(/function search\(query, limit\)\s*\{(?<body>[\s\S]*?)\n\s*return Number/)?.groups?.body ?? "", /ensureCache\(\)|refreshCache\(\)/);
  ...
  assert.match(commandPalette, /onQueryChanged:\s*\{[\s\S]*if \(open\)[\s\S]*appSearch\.requestSearch\(query\);[\s\S]*\}/);
  ...
  assert.match(commandPalette, /function openPalette\(\)\s*\{[\s\S]*resetSearch\(\);[\s\S]*open = true;[\s\S]*appSearch\.ensureCache\(\);[\s\S]*appSearch\.requestSearch\(query\);/);
  ...
  assert.match(commandPalette, /result\.type === "app" && appSearch\.launch\(result\)/);
